...
// Root route
app.get('/', (req, res) => {
  res.send('Women Safety API (WebRTC Enabled) is running...');
});
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 'running',
      message: 'Women Safety API (WebRTC) is running...',
...
// IMPORTANT: Listen on `server` instead of `app` to attach Socket.io
